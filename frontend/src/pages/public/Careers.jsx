...
<div className="space-y-4">{jobs.map((job,i)=><motion.div key={job.JobID} initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} transition={{delay:i*0.04}}>
...
<div><div className="flex items-center gap-2 mb-1"><h3 className="font-bold text-dark-800 group-hover:text-primary-600 transition-colors">{job.Title}</h3>{job.JobType&&<Chip label={job.JobType} size="small" color="primary" variant="outlined"/>}</div><div className="text-gray-500 text-sm flex items-center gap-3">{job.DepartmentName&&<span>🏢 {job.DepartmentName}</span>}{job.Location&&<span>📍 {job.Location}</span>}{job.ExperienceMin&&<span>⏱ {job.ExperienceMin}{job.ExperienceMax?`-${job.ExperienceMax}`:'+'}  yrs exp</span>}</div></div>
